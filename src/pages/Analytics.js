...
const Analytics = () => {
  ...
  const equityData = equityCurve.map((point, index) => ({
    ...
    cumulativeR: parseFloat((point.cumulativeR || 0).toFixed(2)),
    index
  }));
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        ...
        <MetricCard
          ...
          value={`${(averageRMultiple || 0) >= 0 ? '+' : ''}${(averageRMultiple || 0).toFixed(2)}R`}
          subtitle={`Win: ${(averageWinR || 0).toFixed(2)}R | Loss: ${(averageLossR || 0).toFixed(2)}R`}
          color={(averageRMultiple || 0) >= 0 ? 'text-green-600' : 'text-red-600'}
        />
        <MetricCard
          title="Expectancy"
          value={`${(expectancy || 0) >= 0 ? '+' : ''}${(expectancy || 0).toFixed(2)}R`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        <MetricCard
          title="Largest Win"
          value={`+${(largestWin || 0).toFixed(2)}R`}
          subtitle="Best performing trade"
          color="text-green-600"
        />
        <MetricCard
          title="Largest Loss"
          value={`${(largestLoss || 0).toFixed(2)}R`}
