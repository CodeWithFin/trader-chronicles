...
const Analytics = () => {
  ...
  const equityData = equityCurve.map((point, index) => ({
    ...
    cumulativeR: parseFloat(point.cumulativeR.toFixed(2)),
    index
  }));

  // Format strategy win rate data
  const strategyData = Object.entries(winRateByStrategy).map(([strategy, rate]) => ({
    name: strategy,
    winRate: parseFloat(rate.toFixed(2))
  }));

  // Format tag win rate data
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        ...
        <MetricCard
          ...
          subtitle={`${totalTrades > 0 ? ((winRate / 100) * totalTrades).toFixed(0) : 0} winning trades`}
          color="text-green-600"
        />
        <MetricCard
          title="Average R-Multiple"
          value={`${averageRMultiple >= 0 ? '+' : ''}${averageRMultiple.toFixed(2)}R`}
          subtitle={`Win: ${averageWinR.toFixed(2)}R | Loss: ${averageLossR.toFixed(2)}R`}
          color={averageRMultiple >= 0 ? 'text-green-600' : 'text-red-600'}
        />
        <MetricCard
          title="Expectancy"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        <MetricCard
          title="Largest Win"
          value={`+${largestWin.toFixed(2)}R`}
          subtitle="Best performing trade"
          color="text-green-600"
        />
        <MetricCard
          title="Largest Loss"
          value={`${largestLoss.toFixed(2)}R`}
      ...
      <div className="space-y-12">
        ...
        <div className="border-4 border-black bg-white p-6 md:p-12 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
          ...
          <p className="text-sm text-zinc-600 mb-6">Cumulative R-Multiple over time</p>
          {equityData.length > 0 ? (
