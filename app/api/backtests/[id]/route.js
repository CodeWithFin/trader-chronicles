  ...
  try {
    ...
    try {
      rows = await sql.query(
        `SELECT * FROM backtest_trades WHERE id = $1 AND user_id = $2`,
        [params.id, user.id]
      )
    } catch (e) {
    ...
    const data = rows?.[0]
    if (!data) {
      return NextResponse.json({ error: 'Backtest entry not found' }, { status: 404 })
    }
  ...
  try {
    ...
    if (body.pnlAbsolute !== undefined && body.result === undefined) {
      const cur = await sql.query(
        `SELECT result FROM backtest_trades WHERE id = $1 AND user_id = $2 LIMIT 1`,
        [params.id, user.id]
      )
    ...
    if (body.strategyName !== undefined) {
      const strategy = String(body.strategyName).trim()
      push('strategy_name', strategy)
      push('strategy_used', strategy)
    }
    ...
    const query = `
      UPDATE backtest_trades SET ${fields.join(', ')}, updated_at = NOW()
      WHERE id = $${idx} AND user_id = $${idx + 1}
      RETURNING *
    `
    let rows
    try {
      rows = await sql.query(query, values)
    } catch (e) {
      if (isBacktestSchemaMissingError(e)) {
        return NextResponse.json({ error: BACKTESTING_MIGRATION_HELP }, { status: 503 })
      }
...
export async function DELETE(request, { params }) {
  try {
    ...
    const rows = await sql.query(
      `DELETE FROM backtest_trades WHERE id = $1 AND user_id = $2 RETURNING id`,
      [params.id, user.id]
    )
    if (!rows?.length) {
      return NextResponse.json({ error: 'Backtest entry not found' }, { status: 404 })
    ...
    return NextResponse.json({ message: 'Backtest entry deleted successfully' })
  } catch (error) {
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
}
